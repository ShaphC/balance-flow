...
const tx = (id: string, date: string, amount: number, sortOrder = 1000) => ({
  id,
  accountId: "acct",
  userId: "u",
  name: id,
...
describe("cash flow balances", () => {
  it("calculates running balances", () => {
    const result = calculateRunningBalances(4000, [
      tx("rent", "2026-08-01", -1000),
      tx("pay", "2026-08-07", 1000),
    ]);
  ...
  it("calculates monthly totals", () => {
    expect(
      calculateMonthlyTotals(4000, [
        tx("income", "2026-08-01", 2000),
        tx("rent", "2026-08-02", -1200),
      ]),
    ).toEqual({
  ...
  it("rolls an ending balance into the next month", () => {
    const result = calculateMonthChain(
      anchor,
      [tx("income", "2026-08-01", 2000), tx("rent", "2026-08-02", -1200)],
      [],
      "2026-09-01",
    );
  ...
  it("orders same-day transactions by sort order", () => {
    const result = calculateRunningBalances(1000, [
      tx("b", "2026-08-03", -50, 2000),
      tx("a", "2026-08-03", -100, 1000),
    ]);
